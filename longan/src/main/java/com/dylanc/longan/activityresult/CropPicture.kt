...
open class CropPictureContract : ActivityResultContract<CropPictureRequest, Uri?>() {
    ...
    override fun createIntent(context: Context, input: CropPictureRequest): Intent {
        ...
        return Intent("com.android.camera.action.CROP").setDataAndType(input.inputUri, "image/*")
            .putExtra(MediaStore.EXTRA_OUTPUT, outputUri)
            .putExtra("outputFormat", Bitmap.CompressFormat.JPEG.toString())
            .putExtra("return-data", false).putExtras(input.extras).grantReadUriPermission()
    }

    override fun parseResult(resultCode: Int, intent: Intent?): Uri? {
        if (resultCode == Activity.RESULT_OK) return outputUri
        return null
    }
}
