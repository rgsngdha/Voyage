...
/* package */ final class FlacBinarySearchSeeker extends BinarySearchSeeker {
    ...
    public FlacBinarySearchSeeker(FlacStreamMetadata streamMetadata, long firstFramePosition, long inputLength, FlacDecoderJni decoderJni, OutputFrameHolder outputFrameHolder) {
        super(
                /* seekTimestampConverter= */ streamMetadata::getSampleNumber, new FlacTimestampSeeker(decoderJni, outputFrameHolder), streamMetadata.getDurationUs(),
                /* floorTimePosition= */ 0,
                /* ceilingTimePosition= */ streamMetadata.totalSamples,
                /* floorBytePosition= */ firstFramePosition,
                /* ceilingBytePosition= */ inputLength,
                /* approxBytesPerFrame= */ streamMetadata.getApproxBytesPerFrame(),
                /* minimumSearchRange= */ Math.max(FlacConstants.MIN_FRAME_HEADER_SIZE, streamMetadata.minFrameSize));
    ...
    private static final class FlacTimestampSeeker implements TimestampSeeker {
        ...
        @Override
        public TimestampSearchResult searchForTimestamp(ExtractorInput input, long targetSampleIndex) throws IOException {
            ByteBuffer outputBuffer = outputFrameHolder.byteBuffer;
            long searchPosition = input.getPosition();
            decoderJni.reset(searchPosition);
            try {
                decoderJni.decodeSampleWithBacktrackPosition(outputBuffer, /* retryPosition= */ searchPosition);
