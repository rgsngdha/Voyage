...
class SystemNotification constructor(
    ...
    override fun startNotification(songInfo: SongInfo?, playbackState: String) {
        ...
        if (!mStarted) {
            ...
            if (notification != null) {
                ...
                (context as MusicService).customStartForeground(
                    INotification.NOTIFICATION_ID, notification
                )
                mStarted = true
            }
        }
    ...
    private fun createNotification(): Notification? {
        ...
        if (art == null) {
            ...
            if (fetchArtUrl.isNullOrEmpty()) {
                art = BitmapFactory.decodeResource(
                    context.resources, R.drawable.umeng_socialize_share_music
                )
            }
        }
        //适配8.0
        ...
        val smallIcon =
            if (config.smallIconRes != -1) config.smallIconRes else R.drawable.outline_music_note_24
        ...
        if (!config.targetClass.isNullOrEmpty()) {
            ...
            clazz?.let {
                val intent = NotificationUtils.createContentIntent(
                    context, config, songInfo, config.targetClassBundle, it
                )
        ...
        StarrySky.getImageLoader()?.load(fetchArtUrl, object : ImageLoaderCallBack {
            override fun onBitmapLoaded(bitmap: Bitmap?) {
                ...
                mNotificationManager?.notify(
                    INotification.NOTIFICATION_ID, notificationBuilder.build()
                )
            }
    ...
    private fun addActions(notificationBuilder: NotificationCompat.Builder): Int {
        ...
        if (hasPreSong) {
            val icon =
                if (config.skipPreviousDrawableRes != -1) config.skipPreviousDrawableRes else R.drawable.ic_round_skip_previous_24
            val title =
        ...
        if (playbackState == PlaybackStage.PLAYING || playbackState == PlaybackStage.BUFFERING) {
            ...
            icon =
                if (config.pauseDrawableRes != -1) config.pauseDrawableRes else R.drawable.ic_round_pause_24
            intent = mPauseIntent
        } else {
        ...
        if (hasNextSong) {
            val actionIcon =
                if (config.skipNextDrawableRes != -1) config.skipNextDrawableRes else R.drawable.ic_round_skip_next_24
