...
//            .cookieJar(PersistentCookieStore())
    ...
    init {
        okHttpClient = ProgressManager.instance?.with(okHttpClient.newBuilder())?.build()!!
    }
    ...
    fun streamBodyFromFile(filePath: String) =
            RequestBody.create("application/octet-stream".toMediaTypeOrNull(), File(filePath))
    ...
    fun streamBodyFromString(string: String) =
            RequestBody.create("application/octet-stream".toMediaTypeOrNull(), string)
}