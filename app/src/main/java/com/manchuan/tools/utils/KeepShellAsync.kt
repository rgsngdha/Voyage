...
class KeepShellAsync(private var context: Context?, private var rootMode: Boolean = true) :
    ...
    companion object {
        ...
        fun getInstance(key: String, rootMode: Boolean = true): KeepShellAsync {
            synchronized(keepShells) {
                if (!keepShells.containsKey(key)) {
                    keepShells[key] = KeepShellAsync(null, rootMode)
                }
                            ...
                            while (true) {
                                ...
                                if (line != null) {
                                    if (processHandler != null) {
                                        processHandler!!.sendMessage(
                                            processHandler!!.obtainMessage(
                                                PROCESS_EVENT_ERROR_CONTENT, line
                                            )
                                        )
                                    }
                                } else {
    ...
    fun doCmd(cmd: String, isRedo: Boolean = false) {
        try {
            ...
            if (p == null || isRedo || out == null) {
                getRuntimeShell(cmd) {
                    //重试一次
                    if (!isRedo) doCmd(cmd, true)
                    else showMsg("Failed execution action!\nError message : Unable to obtain Root permissions\n\n\ncommand : \r\n$cmd")
                }
            } else {
                out!!.write(cmd)
                out!!.write("\n\n")
