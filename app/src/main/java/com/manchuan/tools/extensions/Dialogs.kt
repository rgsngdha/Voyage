...
import com.lxj.androidktx.NO_GETTER
import com.lxj.androidktx.noGetter
...
val Material: AlertBuilderFactory<DialogInterface> = { context ->
    object : AlertDialogBuilder() {
        override val builder: AlertDialog.Builder = BaseAlertDialogBuilder(context)
    }
}
...
) = alert(defaultAlertBuilderFactory, title, message, block)
...
) = requireContext().alert(factory, title, message, block)
...
fun AlertBuilder<*>.okButton(
    @StringRes okString: Int = android.R.string.ok,
    onClicked: (dialog: DialogInterface) -> Unit,
) = positiveButton(okString, onClicked)
...
fun AlertBuilder<*>.neutralButton(
    @StringRes string: Int = android.R.string.untitled,
    onClicked: (dialog: DialogInterface) -> Unit,
) = neutralPressed(string, onClicked)
...
fun AlertBuilder<*>.cancelButton(
    @StringRes cancelString: Int = android.R.string.cancel,
    onClicked: (dialog: DialogInterface) -> Unit = { it.dismiss() },
) = negativeButton(cancelString, onClicked)
...
abstract class AlertDialogBuilder : AlertBuilder<AlertDialog> {
    ...
    override fun build(): AlertDialog = builder.create().also { dialog ->
        show()
    }
